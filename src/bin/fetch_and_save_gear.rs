...
use dofus_opti::models::{GearType, ALL_GEAR_TYPES};
...
fn save_dofus_db_data(
    ...
    println!("Written {} entry/ies of gear type {} to directory {}", objects.len(), gear_type, dir_name);
    Ok(())
}
...
async fn main() -> Result<()> {
    for gear_type in ALL_GEAR_TYPES {
        ...
        result.iter().take(10).for_each(|x| println!("  {:?}", parse_gear(DofusDbObject::deserialize(x).unwrap())));
